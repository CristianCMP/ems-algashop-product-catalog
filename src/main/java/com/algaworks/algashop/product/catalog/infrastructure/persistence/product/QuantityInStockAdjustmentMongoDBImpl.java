...
public class QuantityInStockAdjustmentMongoDBImpl implements QuantityInStockAdjustment {
    ...
    private Result changeStockQuantity(UUID productId, int quantity, Query queryForUpdate) {
        ...
        Document productBeforeUpdate = mongoOperations.aggregate(findProductQuantity, Product.class, Document.class).getUniqueMappedResult();
...
//        UpdateResult updateResult = mongoOperations.updateFirst(queryForUpdate, update, QuantityInStockAdjustment.class);
//        UpdateResult updateResult = mongoOperations.update(Product.class)
//                .matching(queryForUpdate)
//                .apply(update)
//                .first();
//        Product productUpdated = mongoOperations.findAndModify(queryForUpdate, update, FindAndModifyOptions.options().returnNew(true), Product.class);
        Product productUpdated = mongoOperations.findAndModify(queryForUpdate, update, new FindAndModifyOptions().returnNew(true), Product.class);
